...
{
    internal class QuestDataLocalized
    {
        ...
        public QuestDataLocalized(uint rowId, ClientLanguage language)
            : this(Plugin.DataManager.GetExcelSheet<Quest>(language).GetRow(rowId), language)
        {
        }

        public QuestDataLocalized(Quest quest, ClientLanguage language)
        {
            this.Quest = quest;
            ...
            this.ItemRewards = new ItemRewardsData(quest, this.Language);
        ...
        private void SetPreviousQuests()
        {
            if (this.Quest.PreviousQuest.Count > 0)
            {
                ...
                {
                    Plugin.Log.Debug($"Adding 'The Light of Inspiration' Role Quests as previous ({this.Language})");
        ...
        private ClassJob ParseJobUnlock(Quest quest)
        {
                ...
                (this.ItemRewards.OtherItem!.RowId <= 16 && this.ItemRewards.OtherItem!.RowId >= 10)) // ../csv/QuestRewardOther.csv
            {
                ...
                var item = Plugin.DataManager.GetExcelSheet<Item>(this.Language)!.FirstOrDefault(i => i.Icon == this.ItemRewards.OtherItem.Icon);
        ...
        private List<InstanceData> ParseInstanceUnlocks(Quest quest)
        {
            ...
            if (!quest.IsRepeatable) // Parse Instance Unlocks
            {
                ...
                instanceUnlocks.AddRange(quest.QuestParams
                    ...
                    .Select(qp => new InstanceData(qp, this.Language)));

                if (quest.InstanceContentUnlock.RowId != 0 && !instanceUnlocks.Any(iu => iu.ContentRowId == quest.InstanceContentUnlock.RowId))
                {
                    instanceUnlocks.Add(new InstanceData(quest.InstanceContentUnlock.RowId, this.Language));
                }
            }
